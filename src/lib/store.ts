...
async function readStoreJson<T>(key: string, filePath: string, fallback: T): Promise<T> {
  const fromSql = await sqliteGet<T>(key);
...
async function writeStoreJson<T>(key: string, filePath: string, data: T): Promise<boolean> {
  const sqlOk = await sqliteSet(key, data);
  try {
    await ensureDataDir();
